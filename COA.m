...
while(iter<M)
    ...
    [X_new_com]=combination(lb,ub,X,M,dim,c_percent,pop);
    [X_new_dec]=decomposition(lb,ub,X,dim,pop,x_worest,x_best,c_percent);
    [X_new_rep]=replacement(lb,ub,X,dim,pop,x_worest,x_best,c_percent,Y);
    [X_new_rep_double]=replacement_double(lb,ub,X,dim,pop,x_worest,x_best,c_percent,Y);
...
%% 第一个尝试 全局停止函数
     [iter,YY_overall_stop,t]=Y_stop(iter,fMin,YY_overall_stop,M,t,overall_stop_num); % 全局停止函数
%%
    iter=iter+1;
